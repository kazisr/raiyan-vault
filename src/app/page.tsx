...
export default async function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] text-[var(--on-surface)] antialiased transition-colors duration-300">
      <main className="max-w-4xl px-4 py-8 sm:px-6 mx-auto space-y-6">

        {/* Header */}
        <header className="flex items-center justify-between border-b border-[var(--outline-variant)] pb-6">
          <div>
          ...
          <div className="flex items-center gap-2">
            <ThemeToggle />
            <div className="w-10 h-10 rounded-full bg-[var(--primary-container)] flex items-center justify-center text-[var(--primary)]">
        ...
        {/* Footer */}
        <footer className="flex items-center justify-between pt-6 border-t border-[var(--outline-variant)] text-xs text-[var(--on-surface-muted)]">
          <p>For {CHILD_NICKNAME} with love</p>
          <Link
            href="/login"
            className="flex items-center gap-1.5 hover:text-[var(--on-surface)] transition-colors font-medium p-1 -m-1 rounded-md focus:outline-none focus:ring-2 focus:ring-[var(--primary)]/20"
          >
